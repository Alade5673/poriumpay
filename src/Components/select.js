...
const CustomSelect = ({options, onChange, value}) =>{
    ...
    const [selected, setSelected] = useState({img:null,name:"select", biller:null})
    ...
    return (
       <div>
            <div onClick={()=>setOpen(!open)} className={`w-full flex items-center justify-between p-2 pr-4 text-primary border rounded-xl outline-none text-sm transition duration-150 ease-in-out mb-4 border-blue-700`}>
            ...
            {
             open &&<div className="pl-2 py-4 pr-4 absolute bg-white ml-3 w-10/12 shadow-sm h-52 overflow-y-auto">
                 {
                     options.map((value, index)=>(
                         <div key={index} className="flex items-center space-x-4 pb-2 mb-2 border-b border-gray-100" onClick={()=>_handleSelect({...value})}>
